...
function evalEq(textStr) {
   ...
   const lastLine = lines[lines.length - 1].trim(); // Trim whitespace

   // Check if lastLine is empty or contains invalid characters
   if (lastLine === "" || /[^0-9+\-*/(). ]/.test(lastLine)) {
       return "Error"; // Return an error message for invalid input
   }

   // Evaluate the equation safely
   let eqValue = eval(lastLine);

   return Number(eqValue).toFixed(6); // Ensure numeric precision
}
